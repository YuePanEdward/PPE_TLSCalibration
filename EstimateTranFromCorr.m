...
t_vec=target_grav_cent-(R_mat*source_grav_cent')'; % translation vector (source to target) t_os

% nav toolbox required for the rotm2eul function
% r_vec: [ omega(r_x), phi(r_y), kappa(r_z) ] , (in rad , anti-clockwise)
% R_mat' : R_so
r_vec=rotm2eul(R_mat','XYZ'); % p_s = R_so * (p_o - t_os) = R_x(omega)* R_y(phi) * R_z(kappa) * (p_o - t_os) 
